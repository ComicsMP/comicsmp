...
// Join wanted_items (alias w) with Comics (alias c) so we can get the wanted_items.ID
$sql = "
    SELECT 
       w.ID AS wanted_id,
       c.Image_Path, 
    ...
    JOIN wanted_items w 
     ...
     AND w.Comic_Title = c.Comic_Title 
     AND w.Years = c.Years
    WHERE w.user_id = ? 
      AND c.Comic_Title = ? 
      AND c.Years = ? 
      AND c.Issue_URL IN ($placeholders)
    ORDER BY 
      CAST(
        REGEXP_SUBSTR(
...
while ($row = $result->fetch_assoc()) {
    // Normalize image path (your existing logic)
    ...
    // Process Issue_Number: force a '#' prefix for display.
    ...
    $wanted_id  = htmlspecialchars($row['wanted_id']);
    ...
    // Delete button now carries the unique record id:
    $output .= '<button class="remove-cover" style="position: absolute; top: 5px; right: 5px; background: rgba(255,0,0,0.8); color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 14px; cursor: pointer; line-height: 18px; text-align: center;" data-id="' . $wanted_id . '">&times;</button>';

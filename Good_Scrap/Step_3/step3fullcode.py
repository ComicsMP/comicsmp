        ...
        try:
            ...
            for _, row in data.iterrows():
                ...
                bulk_data.append(new_row)
                 ...
                 # --- NEW: Update series-level Comic_SearchIndex -----------------------
                 comic_title = unify(row.get('Comic_Title'))
                 if comic_title:
                     normalized_title = re.sub(r'[\s\-:()]+', ' ', comic_title).lower().strip()
                     try:
                         cursor.execute("""
                            INSERT IGNORE INTO Comic_SearchIndex (original_title, normalized_title)
                            VALUES (%s, %s)
                         """, (comic_title, normalized_title))
                    except Exception as index_err:
                       print(f"⚠️ Failed to update Comic_SearchIndex for {comic_title}: {index_err}")
# ----------------------------------------------------------------------

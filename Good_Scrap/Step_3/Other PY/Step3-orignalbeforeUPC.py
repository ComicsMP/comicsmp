...
def bulk_insert_dedup(folder_path, db_config):
    ...
    # Normalize Issue_URL and load existing records
    existing_data = {}
    try:
        cursor.execute("SELECT TRIM(LOWER(Issue_URL)) AS Issue_URL, Unique_ID FROM comics")
        for row in cursor.fetchall():
            existing_data[row["Issue_URL"].strip().lower()] = row["Unique_ID"]  # Normalize
    ...
    # Prepare SQL for inserts/updates based on your DB structure.
    upsert_sql = """
    INSERT INTO comics (
        Tab,
        Comic_Title,
    ...
    ON DUPLICATE KEY UPDATE
        ...
        Issues_Note = VALUES(Issues_Note),
    ...
    # Prepare SQL to update comic_title_suggestions
    insert_suggestions_sql = """
    INSERT IGNORE INTO comic_title_suggestions (Comic_Title) VALUES (%s)
    """
    ...
    # Define the default image filename (used to check if a row uses the placeholder)
    ...
    for file_name in excel_files:
        ...
        try:
            ...
            # Normalize Volume (only store the number)
            if 'Volume' in data.columns:
                data['Volume'] = data['Volume'].apply(normalize_volume)

            # Normalize Issue Number (append # if missing)
            ...
            for _, row in data.iterrows():
                # Normalize Issue_URL to lowercase
                issue_url = unify(row.get('Issue_URL'))
                if issue_url:
                    issue_url = issue_url.lower()  # Normalize to lowercase!
                unique_id = unify(row.get('Unique_ID'))
                image_url = unify(row.get('Image_URL'))

                # Determine if this row is using the default image by checking if the Image_URL ends with the default filename.
                uses_default_image = image_url and image_url.lower().endswith(DEFAULT_IMG_FILENAME.lower())

                # Skip if Issue_URL exists and:
                # - For non-default images: the Unique_ID matches.
                # - For default images: the stored Unique_ID is already None.
            ...
            # Verify no new duplicates were inserted
            cursor.execute("""
            SELECT Issue_URL, COUNT(*) 
            FROM comics 
            GROUP BY Issue_URL 
            HAVING COUNT(*) > 1
            """)
            ...
            if duplicates_found:
                print(f"⚠️ WARNING: {len(duplicates_found)} duplicate Issue_URLs detected after processing!")
                for dup in duplicates_found[:5]:  # Show only first 5 duplicates
                    print(f"Duplicate: {dup['Issue_URL']} ({dup['COUNT(*)']} times)")
            else:
                print("✅ No duplicates found after processing.")

            # Insert or update rows in comics.
            if bulk_data:
            ...
            # Insert comic title suggestions (duplicates will be ignored).
            if file_suggestions:
            ...
            if os.path.exists(processed_file_path):
                # Append a timestamp to the file name if it already exists
...
if __name__ == "__main__":
    ...
    folder_path = 'C:/Users/ductm/OneDrive/Desktop/Good Scrap/Step 3'

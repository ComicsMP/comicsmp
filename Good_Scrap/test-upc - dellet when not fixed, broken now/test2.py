...
SEARCH_LIMIT = 10  # Testing with a single database entry

# Path to the FAISS index file
FAISS_INDEX_PATH = r"C:\xampp6\htdocs\comicsmp\FAISS_Mobile_Matching\faiss_index_L2.bin"
# Directory to temporarily save downloaded query images
UPLOADS_DIR = r"C:\xampp6\htdocs\comicsmp\FAISS_Mobile_Matching\uploads"
os.makedirs(UPLOADS_DIR, exist_ok=True)

# Folder where your indexed/matched images are stored (correct folder for visualizing results)
RESULTS_FOLDER = r"C:\xampp6\htdocs\comicsmp\images"

# Path to your metadata file (should contain image file names/paths corresponding to FAISS index)
METADATA_FILE = r"C:\xampp6\htdocs\comicsmp\FAISS_Mobile_Matching\image_metadata.pkl"

# Load metadata (assumed to be a list where each element's first item is the image file name)
...
cursor.close()
connection.close()
print(f"[1] Retrieved {len(results)} entry(ies) to test.")
...
for i, (title, issue, year, date) in enumerate(results):
    ...
    try:
        ...
        # For demo purposes, only process the first issue link
        issue_link = issue_links[0]
        print(f"[3.{i+1}.1] Visiting issue page: {issue_link}")
        ...
        # Check that the page contains the expected title and issue number
        if title.lower() not in text_blocks.lower() or f"#{issue_num}" not in text_blocks:
            print(f"[3.{i+1}.1] Skipping page because title or issue number does not match expected values.")
            continue

        barcode_match = re.search(r'(UPC|Barcode)(?:/EAN)?:?\s*(\d{12,14})', text_blocks, re.IGNORECASE)
        if barcode_match:
            code = barcode_match.group(2)
            print(f"[3.{i+1}.1] Found Barcode/UPC: {code}")
        else:
            print(f"[3.{i+1}.1] Barcode/UPC not found.")

        img_tag = issue_soup.find('img', {'class': 'cover_img'})
        if img_tag:
            img_url = img_tag.get("src")
            print(f"[3.{i+1}.1] Found cover image: {img_url}")

            try:
                img_res = scraper.get(img_url, stream=True, timeout=10)
                if img_res.status_code != 200:
                    print(f"[3.{i+1}.1] Failed to download image, status code: {img_res.status_code}")
                    continue

                filename = os.path.join(UPLOADS_DIR, os.path.basename(img_url.split('?')[0]))
                with open(filename, 'wb') as f:
                    f.write(img_res.content)
                print(f"[3.{i+1}.1] Image saved to: {filename}")

                try:
                    query_img = Image.open(filename).convert('RGB')
                    print(f"[3.{i+1}.1] Query image opened successfully.")
                except Exception as open_e:
                    print(f"[3.{i+1}.1] Error opening saved query image: {open_e}")
                    continue

                # FAISS Matching Step
                feature = extract_features(query_img)
                feature = np.expand_dims(feature, axis=0)
                k = 1
                distances, indices = index.search(feature, k)
                top_match_index = indices[0][0]
                top_distance = distances[0][0]
                print(f"[3.{i+1}.1] Top FAISS match index: {top_match_index}, distance: {top_distance}")

                # Retrieve matched image file from metadata
                matched_filename = os.path.basename(metadata_list[top_match_index][0])
                matched_image_path = os.path.join(RESULTS_FOLDER, matched_filename)
                if not os.path.exists(matched_image_path):
                    print(f"[3.{i+1}.1] Matched image not found at: {matched_image_path}")
                else:
                    try:
                        matched_img = Image.open(matched_image_path).convert('RGB')
                    except Exception as e:
                        print(f"[3.{i+1}.1] Error opening matched image: {e}")
                        continue
                    ...
                    plt.title(f"Matched Image\n(distance: {top_distance:.3f})")
                    plt.axis("off")
                    
                    plt.show()
                    
            except Exception as img_e:
                print(f"[3.{i+1}.1] Error downloading or processing image: {img_e}")
        else:
            print(f"[3.{i+1}.1] Cover image not found.")
    except Exception as e:

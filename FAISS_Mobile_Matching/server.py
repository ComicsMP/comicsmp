...
# -------------------- Login Route Using Email and bcrypt Verification --------------------
...
def login():
    ...
    try:
        ...
        # Query the users table by email and retrieve the stored hash.
        query = """
...
def process_mobile_image(image_path):
    try:
        ...
        with Image.open(image_path) as img:
            ...
            # ---- STEP 1: Apply CLAHE (adaptive contrast) ----
            ...
            # ---- STEP 2: Detect and suppress glare zones ----
            gray = cv2.cvtColor(enhanced, cv2.COLOR_RGB2GRAY)
            glare_mask = cv2.inRange(gray, 240, 255)  # bright reflection threshold
            glare_mask = cv2.dilate(glare_mask, np.ones((5, 5), np.uint8), iterations=1)

            # Apply blur to glare zones only
            blurred = cv2.GaussianBlur(enhanced, (11, 11), 0)
            glare_removed = np.where(glare_mask[:, :, None] == 255, blurred, enhanced)

            # Convert back to PIL for rest of pipeline
            img = Image.fromarray(glare_removed)

      ...
      pendingMatches.forEach(matchObj => {
        ...
        .then(data => {
          if (data.insert_id) {
            ...
            // If all pending comics are submitted, show final confirmation
            if (matches.every(m => m.submitted === true)) {
              showFinalConfirmation(matches.filter(m => m && m.submitted).length);
            }
          } else {

...
<head>
  ...
  <style>
    ...
    /* Additional styles for Dashboard and Profile can be added here */
  </style>
</head>
<body class="bg-light">
  <!-- HEADER -->
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      function performSearch() {
        ...
        $.ajax({
          ...
          success: function(html) {
            ...
            // Attach action buttons for each gallery item.
      ...
      // Listen for clicks on both .gallery-item img and .cover-img (for wanted/sale covers).
      $(document).on("click", ".gallery-item img, .cover-img", function(e) {
        if ($(e.target).closest("button").length) return;
        const parent = $(this).closest(".gallery-item");
        // If not found, try the closest .cover-container (for wanted/sale covers)
        const container = parent.length ? parent : $(this).closest(".cover-container");
        const fullImageUrl = $(this).attr("src");
        const comicTitle = container.data("comic-title") || "N/A";
        const years = container.data("years") || "N/A";
        const issueNumber = container.data("issue-number") || "N/A";
        const tab = container.data("tab") || "N/A";
        const variant = container.data("variant") || "N/A";
        const date = container.attr("data-date") || "N/A";
        ...
        $("#popupDate").text(date);
        const upc = container.data("upc") || "N/A";
        $("#popupUPC").text(upc);

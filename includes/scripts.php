...
$(document).on('click', '#expandFavoritesBtn', function (e) {
  e.preventDefault();
  const $btn = $(this);
  const $hidden = $('.extra-favorite');

  if ($hidden.is(':visible')) {
    $hidden.slideUp(200, () => $hidden.addClass('d-none'));
    $btn.text('Show All');
  } else {
    $hidden.removeClass('d-none').slideDown(200);
    $btn.text('Hide');
  }
});
...
function initMatchesFiltering() {
  ...
  filterTestRows(); // run once on load
    ...
    // ————————
  // clicking a favorite in the offcanvas
  // ————————
  ...
  // 1) Fill in the title field & clear suggestions:
  $('#comicTitle').val(title);
  $('#suggestions').empty();

  // 2) Fetch *that* series' years, populate the dropdown, then pick the correct one:
  $.get('getYears.php', {
    comic_title: title,
    country:     $('#countrySelect').val()
  }, function(html){
    // inject the <option>s and show the year filter
    $('#yearSelect')
      .html('<option value="">Select a year</option>' + html)
      .val(year);
    $('#yearFilterGroup').show();

    // 3) Now that title+year are set, rebuild your tabs bar:
    updateTabButtons(title);

    // 4) slide-up the offcanvas and kick off the search
    bootstrap.Offcanvas.getInstance(
      document.getElementById('searchFiltersOffcanvas')
    ).hide();
...
// BULK EDIT SALE
  ...
  $("#bulkComicTitleDashboard").val(comicTitle);
  $("#bulkYearsDashboard").val(years);
  $("#bulkPriceDashboard").val("");       // Clear previous
  $("#bulkConditionDashboard").val("");   // Clear previous
  const bulkModal = new bootstrap.Modal(document.getElementById("bulkEditModalDashboard"));
  bulkModal.show();
});


  $(document).on("submit", "#bulkEditFormDashboard", function (e) {
    e.preventDefault();
    const form = $(this);
    $.ajax({
...
$(document).on("click", ".gallery-item img", function(e) {
  ...
  // Hide condition/graded/price if not needed
  ...
  $(document).on("click", ".cover-img", function(e) {
    ...
    $("#coverPopupModal").data("issueUrl", $wrapper.data("issueUrl") || $wrapper.attr("data-issue-url") || "");

...
function calculateDistance($lat1, $lon1, $lat2, $lon2) {
    ...
    $c = 2 * asin(sqrt($a));
    $radius = ($distanceUnit === 'km') ? 6371 : 3956;
    return round($c * $radius) . ' ' . $distanceUnit;
}

require_once '../db_connection.php';

// Fetch user's distance preferences
$prefStmt = $conn->prepare("SELECT default_max_radius, distance_unit FROM users WHERE id = ?");
$prefStmt->bind_param("i", $userId);
$prefStmt->execute();
$prefRes = $prefStmt->get_result();
$pref = $prefRes->fetch_assoc();
$prefStmt->close();

$defaultRadius = isset($pref['default_max_radius']) ? (int)$pref['default_max_radius'] : 750;
$distanceUnit  = in_array($pref['distance_unit'] ?? '', ['km', 'mi']) ? $pref['distance_unit'] : 'mi';
...
<div id="matchesContainer">
  ...
  <?php else: ?>
    ...
    <div class="mb-3 d-flex flex-wrap justify-content-end align-items-center">
      ...
      <label class="me-2 mb-0 fw-bold" for="distanceSlider">
  Max Radius:
  <i class="bi bi-info-circle text-muted" data-bs-toggle="tooltip" data-bs-placement="top" title="Measured as the crow flies (straight-line distance)."></i>
</label>

      <span id="distanceValue" class="me-2"><?= htmlspecialchars($defaultRadius) ?></span><?= htmlspecialchars($distanceUnit) ?>&nbsp;
      <input type="range" id="distanceSlider" class="form-range me-4" min="0" max="1000" value="<?= htmlspecialchars($defaultRadius) ?>" style="width:250px;">
    ...
    <table class="table table-striped" id="matchesTable">
      ...
      <?php foreach ($groupedMatches as $otherId => $group):
          ...
          $buyArr   = array_values(array_filter($group, fn($m)=>$m['buyer_id']==$userId));
          $sellArr  = array_values(array_filter($group, fn($m)=>$m['seller_id']==$userId));
          $buyCount = count($buyArr);
          $sellCount= count($sellArr);
          $typeText = "{$buyCount} Buy / {$sellCount} Sell";
          $rawPay   = ($first['buyer_id']==$userId) ? $first['seller_payment'] : $first['buyer_payment'];
          $payment  = $rawPay ? implode(', ',array_map('trim',explode(',',$rawPay))) : '-';
          $rawEx    = ($first['buyer_id']==$userId) ? $first['seller_transaction'] : $first['buyer_transaction'];
          $exchange = $rawEx ? implode(', ',array_map('trim',explode(',',$rawEx))) : '-';
          $intent   = $buyCount && !$sellCount ? 'buy' : ($sellCount && !$buyCount ? 'sell' : 'buy_sell');
      ...
      ?>
        <tr class="main-row"
          ...
          <td>
            <!-- Added data-intent, data-displayname, data-buy-matches & data-sell-matches -->
            <button
              class="btn btn-sm btn-outline-secondary pm-btn"
              data-user-id="<?= $otherId ?>"
              data-intent="<?= $intent ?>"
              data-displayname="<?= htmlspecialchars($name) ?>"
              data-buy-matches='<?= htmlspecialchars(json_encode($buyArr), ENT_QUOTES) ?>'
              data-sell-matches='<?= htmlspecialchars(json_encode($sellArr), ENT_QUOTES) ?>'
            >
  ...
  <td colspan="9">
    ...
    <style>
  .match-details-table th.cover-col,
  .match-details-table td.cover-col {
    width: 50px;
  }
  .match-details-table th.title-col,
  .match-details-table td.title-col {
    width: 200px;
  }
  .match-details-table th.year-col,
  .match-details-table td.year-col {
    width: 100px;
  }
  .match-details-table th.issue-col,
  .match-details-table td.issue-col {
    width: 80px;
  }
  .match-details-table th.condition-col,
  .match-details-table td.condition-col {
    width: 130px;
  }
  .match-details-table th.price-col,
  .match-details-table td.price-col {
    width: 120px;
  }
</style>

<?php if (!empty($buyArr)): ?>
  <div class="mb-2 fw-bold text-primary">Buy</div>
  <table class="table table-bordered table-sm mb-3 match-details-table">
...
/>
        ...
        <td class="title-col"><?= htmlspecialchars($m['comic_title']) ?></td>
        <td class="year-col"><?= htmlspecialchars($m['years']) ?></td>
        <td class="issue-col"><?= htmlspecialchars($m['issue_number']) ?></td>
        <td class="condition-col"><?= htmlspecialchars($cond) ?></td>
        <td class="price-col"><?= '$'.htmlspecialchars($price).' '.htmlspecialchars($m['seller_currency']??'') ?></td>
      </tr>
    <?php endforeach; ?>
    </tbody>
  </table>
...
<?php if (!empty($sellArr)): ?>
  <div class="mb-2 fw-bold text-success">Sell</div>
  <table class="table table-bordered table-sm match-details-table">
    <thead class="table-light">
      <tr>
        <th class="cover-col">Cover</th>
        <th class="title-col">Title</th>

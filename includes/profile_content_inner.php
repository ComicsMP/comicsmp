...
// Fetch user data (now including default_max_radius and distance_unit)
$query = "SELECT username, email, phone, city, bio, profile_picture, joined_date,
                 preferred_currency,
                 default_max_radius, distance_unit,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['nickname'], $_POST['email'], $_POST['city'], $_POST['preferred_currency'])) {
        ...
        // New: get default radius & unit
        $default_max_radius = isset($_POST['default_max_radius'])
            ? intval($_POST['default_max_radius'])
            : 750;
        $distance_unit = in_array($_POST['distance_unit'] ?? '', ['mi','km'])
            ? $_POST['distance_unit']
            : 'mi';

        // Updated query to include default_max_radius and distance_unit
  ...
  <form id="profileForm" method="POST" enctype="multipart/form-data">
    <div class="accordion" id="profileAccordion">
          ...
          <div class="accordion-body">
            <div class="mb-3">
              ...
              <div id="locationStatus"
                   style="margin-top:5px;font-size:0.9em;color:#007bff;">
              </div>
            </div>
            <input type="hidden" name="latitude" id="latitude"
                   value="<?= htmlspecialchars($user['latitude'] ?? '') ?>">
            <input type="hidden" name="longitude" id="longitude"
                   value="<?= htmlspecialchars($user['longitude'] ?? '') ?>">
            <div class="mb-3">
      ...
      <div class="accordion-item mb-3">
        ...
        <div id="collapsePreferences" class="accordion-collapse collapse"
             aria-labelledby="headingPreferences"
             data-bs-parent="#profileAccordion">
          <div class="accordion-body">
            <div class="mb-3">
              ...
              <select name="preferred_currency" class="form-select">
                <?php foreach ($currencies as $curr): ?>
                  <option value="<?= $curr ?>"
                    <?= ($user['preferred_currency'] === $curr) ? 'selected' : '' ?>>
                    <?= $curr ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <!-- New: Default Max Radius -->
            <div class="mb-3">
              <label class="form-label">Default Max Radius</label>
              <div class="input-group">
                <input type="number"
                       name="default_max_radius"
                       class="form-control"
                       min="0" max="1000"
                       value="<?= htmlspecialchars($user['default_max_radius'] ?? 750) ?>"
                       required>
                <span class="input-group-text">
                  <?= ($user['distance_unit'] ?? 'mi') === 'km' ? 'km' : 'mi' ?>
                </span>
              </div>
            </div>
            ...
            <!-- New: Distance Unit -->
            <div class="mb-3">
              <label class="form-label">Distance Unit</label><br>
              <div class="form-check form-check-inline">
                <input class="form-check-input"
                       ...
                       id="unitMi"
                       value="mi"
                       <?= ($user['distance_unit'] ?? 'mi') === 'mi' ? 'checked' : '' ?>>
                <label class="form-check-label" for="unitMi">Miles</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input"
                       type="radio"
                       ...
                       id="unitKm"
                       value="km"
                       <?= ($user['distance_unit'] ?? 'mi') === 'km' ? 'checked' : '' ?>>
                <label class="form-check-label" for="unitKm">Kilometers</label>
              </div>
            </div>
...
$("#profileForm").submit(function(e) {
  ...
  $.ajax({
    url: '', // handled by this same script
    type: 'POST',
    data: formData,
    contentType: false,
    processData: false,
    success: function() {
      $("#profileContentContainer").prepend(
        '<div class="alert alert-success" id="flashMessage">Profile updated successfully.</div>'
      );
      setTimeout(function(){
        $("#flashMessage").fadeOut(500, function(){ $(this).remove(); });
      }, 3000);
      $.get("includes/profile_content_inner.php", { t: Date.now() }, function(html) {
...
$("#detectLocation").click(function(){
  $("#locationStatus").text("Getting your location, please wait...");
        ...
        geocoder.geocode({ location: {lat, lng} }, function(results, status){
          if (status==="OK" && results[0]) {
            ...
            if (city) {
              ...
              $("#latitude").val(lat);
              $("#longitude").val(lng);
              $("#locationStatus").text("Location detected: "+city);
              return;
            }
          }
          $("#locationStatus").text("");
